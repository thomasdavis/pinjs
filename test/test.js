...
if (key.length > 0) {
  ...
  describe('Get recipient data', function() {
    it('should return successfully', function(done) {
      pin.getRecipientData(testRecipient.token, function(err, res) {
        ...
        if (JSON.stringify(res) !== JSON.stringify(testRecipient)) {
          throw new Error('get recipient data failed')
        }
        done();
      });
    })
  ...
  describe('Update recipient data', function() {
    it('should update email', function(done) {
      pin.updateRecipientData(testRecipient.token, {
        email: 'test@test.com'
      }, function(err, res) {
        if (err) {
          throw err
    ...
    it('should update recipient bank account', function(done) {
      var bankUpdate = {
        "name": "Mr SLim Pterodactyl",
        "bsb": "432412",
        "number": "987654321"
      }
      pin.updateRecipientData(testRecipient.token, {
        email : 'email@email.com',
        bank_account: bankUpdate
      }, function(err, res) {
        if (err) {
          throw err
        };
